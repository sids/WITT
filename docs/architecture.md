...
- `QRCode` can bind only to an Area or Container. A bound code has exactly one target in the same Place; an unbound code has no target. The first-release repository contract permits exactly one active QR per Area or Container and exactly one active target per payload.
...
`QRCodeResolving` and [`QRCodeResolution`](../witt/QR/QRCodeResolution.swift) isolate token lookup from routing. The repository returns known Area, known Container, unknown, needs-repair, or conflict. Duplicate rows for one target resolve as repairable instead of being mistaken for a healthy binding. Ordinary binding is idempotent only for the same token/target pair; otherwise a token already in storage or a target with an existing bound code is rejected. `createTargetAndBindQRCode` creates or selects the Room and Area, optionally creates or selects a Container, and binds the token in one Core Data transaction.

QR resolution is deliberately read-only. The deployed Core Data and CloudKit schema still contains the optional `QRCode.lastScannedAt` attribute, but the runtime exposes no typed accessor and never updates it. Retaining the inert field preserves additive schema compatibility; retiring its behavior avoids a shared-store write on every physical scan, ambiguous cross-device recency, and access-history semantics the product does not use. Legacy values remain untouched.

`CreateAreaDraft` and `CreateContainerDraft` may carry an unused QR token, allowing the target and binding to be saved atomically with its name, detail, photo, and parent. Explicit replacement is also transactional: it accepts an unused token, deletes the target's former QR rows, and binds the new token in the target's persistent store. The former label then resolves as unknown and can be reused. A token attached healthily to any other target is rejected without mutation; WITT never silently moves a binding across targets or Places.

Repair operations accept only `needsRepair` or conflict resolutions. Existing-target repair deletes every row for the damaged payload and every stale row on the selected eligible target, then inserts one valid binding in the target's store as one transaction. Create-and-repair performs destination creation and the same consolidation atomically. The general Repair QR surface refuses a target with a different healthy code. A distinct contextual repair-and-replace operation is available only after the user scans from a specific target's Attach/Reattach action and confirms replacement; this preserves the intended reattach behavior without weakening general takeover protection. Draft repair can release damaged rows before the normal target-save transaction, while a later race is still rejected by ordinary binding rules.

[`QRDeepLinkRouter`](../witt/App/QRDeepLinkRouter.swift) converts known destinations to add-Thing routes, unknown tokens to attach routes, and damaged/conflicting resolutions to a `QRCodeRepairRoute` that retains the token and repair details. QR identity belongs only to Areas and Containers; Rooms and Things are intentionally outside the binding model.
...
Each Place's Rooms screen reads the store's cached CloudKit sharing state to expose system share management contextually. Collection presentations remain domain-specific: Rooms use landscape tiles, Storage Areas use image-led rows, and Things and Containers use square media tiles with restrained text overlays. Room, Thing, and Container grids use two columns at standard Dynamic Type sizes and one column at accessibility sizes so creation labels and item names remain readable without truncation. Descendant counts walk the active snapshot graph and deduplicate Thing identifiers so malformed repeated paths do not inflate totals. Thing details and management forms also resolve current snapshots by ID, so a reload does not leave a view holding a stale managed object. Creation forms use local focus state to focus the primary name field without applying that behavior to edit forms.

Creation and editing are routed through `ManagementRoute` and one-screen forms in [`Features/Management`](../witt/Features/Management). Draft state is converted to typed repository mutations only on commit. The two Thing-creation surfaces share one `AIAssistedThingFormState` for request identity, pending manual edits, suggestion provenance, stale-result rejection, and failure state while retaining their distinct presentation. New Storage Area and Container forms can hold a scanned unused token until the complete draft is saved atomically. Context-aware destination choices are derived from the active Place graph; Container choices exclude the edited Container and its descendants. Archive confirmation copy uses computed subtree impact. Unknown QR attachment uses dedicated state in [`AttachQRCodeView.swift`](../witt/Features/Scan/AttachQRCodeView.swift), showing unassigned Areas and Containers first and falling through to the atomic create-and-bind flow.

AI remains behind `ThingPhotoLabelingService`. The Responses-compatible adapter is the current local/integration seam. The planned production relay/auth contract and evaluation gates are documented in [ai-production.md](ai-production.md), but no unused relay client or synthetic evaluator is compiled into the app before the backend, authentication strategy, provider, privacy posture, and real consented corpus are selected.
...
- the AI interface, deterministic Debug fallback, Responses-compatible request/response handling, configuration selection, and error mapping;
- management preselection, shared AI-assisted form state, post-save dismissal handoffs, exact destination reconstruction, archive facts, adaptive catalog presentation, and Place-sharing helpers.
