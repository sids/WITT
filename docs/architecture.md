...
AI remains behind `ThingPhotoLabelingService`. The Responses-compatible adapter is the current local/integration seam. An unwired `RelayThingPhotoLabelingService` now prepares the production mobile contract: HTTPS only, short-lived scoped credentials, ephemeral no-cache/no-cookie transport, redirect rejection, zero-retention request intent, bounded correlated JSON responses, and no client-selected model or prompt. `ThingPhotoLabelingEvaluator` provides deterministic per-case scoring, while the committed manifest contains only synthetic observations and opaque image references. The real consented photo corpus, relay/auth deployment, provider selection, privacy approval, and runtime wiring remain release gates documented in [ai-production.md](ai-production.md).
