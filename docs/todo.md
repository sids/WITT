...
- [x] FB-017 - Sid, July 11, 2026, build `1.0 (3)`: Replace the tab bar and Places root with a Mail-style system bottom toolbar over one active Place's Browse stack. Browse opens by default with the last valid Place selected. The leading menu uses the `mappin.and.ellipse` SF Symbol, switches Places, and creates a new Place; retain access to Print QR Labels after removing the Places screen. System toolbar search occupies the center and searches the full catalogue. A trailing Scan button opens the camera as a full-screen overlay with an explicit close button that returns to the unchanged Browse position. If no Place exists, lead directly into New Place. Preserve selected Place and deepest in-Place Browse destination across launches, use standard iOS 26 toolbar/search controls without custom glass styling, and share iPhone/iPad screenshots before completion.

FB-015 through FB-017 are implemented and locally verified for the next TestFlight build. The current simulator suite passes with 125 tests, and final Browse, active Search, scanner, and responsive toolbar states were checked on iOS 26 iPhone and iPad simulators.
