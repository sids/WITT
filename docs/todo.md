...
- [x] FB-033 - Sid, July 13, 2026, build `1.0 (6)`: Removed the dark artifacts at the populated New Storage Area row's bottom corners. Its formerly transparent `List` background exposed dark backing pixels through the antialiased section mask; the populated row now uses the adaptive secondary grouped system surface while the empty state remains clear. Verified clean rounded corners and unchanged dashed geometry in light and dark appearance on an iOS 26 iPhone, plus all 145 tests in Debug and Release-optimized configurations with zero failures or skips; screenshots were shared with Sid.
