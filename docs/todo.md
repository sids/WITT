...
- [x] FB-033 - Sid, July 13, 2026, build `1.0 (6)`: Corrected the populated New Storage Area row after rejecting the adaptive filled-row attempt in commit `25a4b24` and the misaligned inset-corner treatment in commit `a866057`. The row remains transparent with a native-looking separator and no dashed top edge. Its sides, rounded bottom corners, and bottom edge now share one continuous four-point-inset path and one-point tint stroke; a same-path corner mask prevents dash phase from blanking either curve without changing its geometry or weight. The empty-state treatment is unchanged. Verified both corner joins in light and dark appearance on an iOS 26 iPhone at the 368 x 800 review scale, then passed all 145 tests in Debug and Release-optimized configurations with zero failures or skips.
