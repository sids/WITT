...
- Contextual creation controls continue through Rooms, Storage Areas, and Containers. New Room, Thing, and Container actions are tint-matched dashed tiles inside their grids. New Storage Area is a fully rounded standalone dashed row when the Room is empty; after Storage Areas exist, it becomes a transparent attached final row with a native separator above it and no dashed top edge. A single uniformly inset, one-point tint contour draws its dashed sides, rounded bottom corners, and bottom edge continuously inside the native List mask. Creation sheets focus their primary name field immediately.
...
Version 1.0 build 6 is `IN_BETA_TESTING` through the `WITT Internal` TestFlight group. It adds the corrected dashed creation corners, Room action-menu polish, Place-scoped Print QR Labels action, and physical label-paper printing model from FB-025 through FB-028. Its source commit is `66fe38f`, and its App Store Connect build ID is `fdd162af-54ce-4cc4-967c-eb6296dc9966`. The arbitrary-QR implementation and product expansion in commit `71e6551` postdate this archive and require a later TestFlight build. The affected iPhone still needs to confirm manual Container creation before FB-024 closes. See [release.md](release.md) for durable release facts and process.
