...
- `docs/ai-production.md` - planned relay/auth boundary, evaluation gates, and rollout safeguards.
...
Current capabilities, verification counts, and TestFlight state are intentionally not duplicated here. Read `docs/status.md` for the implementation snapshot, `docs/release.md` for distribution facts, and `docs/todo.md` for active work. Keep those documents current instead of expanding this bootstrap with volatile details.

Live AI activation still requires a WITT-owned relay or another secure short-lived credential strategy, plus a chosen model and privacy policy; never ship a long-lived provider API key in the iOS app. The next product-critical validation remains the real-device, two-iCloud-account sharing spike, especially `PhotoAsset` binary transfer and bidirectional edits.
